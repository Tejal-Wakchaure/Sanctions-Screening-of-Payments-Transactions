			...
			while (iterator.hasNext()) {
				...
				while (cells.hasNext()) {
					...
					case 0:
						...
						if (isfieldValid) {
							...
							if (ref.isEmpty() || ref.isBlank() || ref.equals(null)) {
								t.setRemark(" No Transaction Reference ID Provided. Please Dont keep it Null! ");
					...
					case 1:
						...
						if (dateString.isBlank() || dateString.isEmpty() || dateString.equals(null)) {
							...
							t.setRemark(t.getRemark() + " Date Not Provided, Please Dont keep it Null! ");
							logger.error("Date Not Provided, Please Dont keep it Null!");
						} else {
					...
					case 2:
						...
						if (isfieldValid) {
							...
							if (name.isBlank() || name.isEmpty()) {
								if (!(t.getRemark().length() < 1))
									t.setRemark(t.getRemark() + ",");
								t.setRemark(t.getRemark() + " Payer Name Not Provided, Please Dont keep it null ");
								t.setPayerName("-");
							} else {
								t.setPayerName(name);
							}
					...
					case 4:
						...
						if (isfieldValid) {
							...
							if (name2.isBlank() || name2.isEmpty()) {
								if (!(t.getRemark().length() < 1))
									t.setRemark(t.getRemark() + ",");
								t.setRemark(t.getRemark() + " Payeee name Not Provided, Please Dont keep it null  ");
								t.setPayeeName("-");
							} else {
								t.setPayeeName(name2);
							}
				...
				if (invalideFieldCount > 0) {
					...
					if (cid != 7) {
						if (!(t.getRemark().length() < 1))
							t.setRemark(t.getRemark() + ",");
						t.setRemark(t.getRemark() + " Please set data,Null field not accepted! ");
						t.setStatusValidate("Fail");
						t.setStatusScreen("NA");
